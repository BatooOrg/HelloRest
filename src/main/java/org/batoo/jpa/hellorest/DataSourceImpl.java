 ...
 * @author wallacew
 */
public class DataSourceImpl extends BoneCPConfig implements DataSource, ObjectFactory {
	...
	private final Map<UsernamePassword, BoneCPDataSource> multiDataSource = new MapMaker().makeComputingMap(new Function<UsernamePassword, BoneCPDataSource>() {
		...
		public BoneCPDataSource apply(UsernamePassword key) {
			...
			ds = new BoneCPDataSource(DataSourceImpl.this.getConfig());
	 ...
	 *            the configuration
	 */
	public DataSourceImpl(BoneCPConfig config) {
	...
	private void maybeInit() throws SQLException {
		...
		DataSourceImpl.logger.debug(this.toString());
